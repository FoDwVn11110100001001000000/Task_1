        ...
        for key, value in name_folders.items():
            if i.lower().endswith(key):
                try:
                    ...
                    translit = translit(file_path, reversed=True)
                    if value == 'Archive':
                        shutil.unpack_archive(f'{directory}/{translit}', f'{directory}/Archive/{translit[:-4]}')
                        os.remove(f'{directory}/{translit}')
                    else:
                        shutil.move(f'{directory}/{translit}', f'{directory}/{value}/{translit}')
                except:
                    if value == 'Archive':
                        shutil.unpack_archive(f'{directory}/{file_path}', f'{directory}/Archive/{file_path[:-4]}')
                        os.remove(f'{directory}/{file_path}')
                    else:
